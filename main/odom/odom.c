...
static TaskHandle_t s_odom_task_h = NULL;

static EventGroupHandle_t s_odom_evt_group = NULL;
#define ODOM_RUN_BIT BIT0
#define ODOM_TERM_BIT BIT1

static EventGroupHandle_t s_odom_err_evt_group;
static EventBits_t s_odom_err_bit;

static odrive_context_t *s_odrive_ml_context;
static odrive_context_t *s_odrive_mr_context;
static servo_t *s_servo_context;

static void odom_task(void *pv) {
    ESP_UNUSED(pv);
    ESP_LOGI(ODOM_TASK_LOGGER_TAG, "Odom task started");

    EventBits_t bits = xEventGroupWaitBits(s_odom_evt_group, ODOM_RUN_BIT, pdFALSE, pdTRUE, portMAX_DELAY); // Wait for start signal
    ...
    if (mros_init_odometry_msg(&odom_msg) != ESP_OK) {
        ESP_LOGE(ODOM_TASK_LOGGER_TAG, "Failed to initialize odometry message");
        vTaskDelete(NULL);
    }
    xEventGroupClearBits(s_odom_evt_group, ODOM_TERM_BIT);
    ...
    if (odrive_get_encoder_estimates(s_odrive_ml_context, &estimates_ml, &timestamp_ml, false, pdMS_TO_TICKS(S_TO_MS(10))) != ESP_OK) {
        ESP_LOGE(ODOM_TASK_LOGGER_TAG, "Failed to get encoder estimates for ODrive Node ID %d", s_odrive_ml_context->node_id);
        xEventGroupSetBits(s_odom_err_evt_group, s_odom_err_bit);
        vTaskDelete(NULL);
    }
    if (odrive_get_encoder_estimates(s_odrive_mr_context, &estimates_mr, &timestamp_mr, false, pdMS_TO_TICKS(S_TO_MS(10))) != ESP_OK) {
        ESP_LOGE(ODOM_TASK_LOGGER_TAG, "Failed to get encoder estimates for ODrive Node ID %d", s_odrive_mr_context->node_id);
        xEventGroupSetBits(s_odom_err_evt_group, s_odom_err_bit);
        vTaskDelete(NULL);
    }
    ...
    while (true) {
        bits = xEventGroupGetBits(s_odom_evt_group);
        if (!(bits & ODOM_RUN_BIT)) {
            break;
        }
        ...
        if (forward_kinematics(&fk_input, &fk_output) != ESP_OK) {
            ESP_LOGE(ODOM_TASK_LOGGER_TAG, "Failed to calculate forward kinematics");
            break;
        }
        ...
        if (mros_update_odometry(&odom_msg) != ESP_OK) { // Mros will publish it once the internal timer is triggered
            ESP_LOGE(ODOM_TASK_LOGGER_TAG, "Failed to update odometry");
            break;
        }
        vTaskDelay(1);
    }
    ...
    if ((bits & ODOM_RUN_BIT)) {
        xEventGroupSetBits(s_odom_err_evt_group, s_odom_err_bit);
        ESP_LOGE(ODOM_TASK_LOGGER_TAG, "Odom task terminated unexpectedly");
    }

    xEventGroupSetBits(s_odom_evt_group, ODOM_TERM_BIT);
    ESP_LOGI(ODOM_TASK_LOGGER_TAG, "Odom task stopped");
    s_odom_task_h = NULL;
    vTaskDelete(NULL);
}
...
esp_err_t odom_task_init(odrive_context_t *odrive_ml_context, odrive_context_t *odrive_mr_context, servo_t *servo_context, EventGroupHandle_t error_handle, EventBits_t error_bit) {
    if (!error_handle) {
        ESP_LOGE(ODOM_TASK_LOGGER_TAG, "Invalid error handle");
        return ESP_ERR_INVALID_ARG;
    }

    if (odrive_ml_context == NULL || odrive_mr_context == NULL) {
        ESP_LOGE(ODOM_TASK_LOGGER_TAG, "Invalid ODrive context");
        return ESP_ERR_INVALID_ARG;
    }

    if (odrive_ml_context->node_id == odrive_mr_context->node_id) {
        ESP_LOGE(ODOM_TASK_LOGGER_TAG, "ODrive Node IDs are the same");
        return ESP_ERR_INVALID_ARG;
    }
    ...
    if (odrive_ml_context->control_mode != CONTROL_MODE_VELOCITY_CONTROL || odrive_mr_context->control_mode != CONTROL_MODE_VELOCITY_CONTROL) {
        ESP_LOGE(ODOM_TASK_LOGGER_TAG, "ODrive is not in velocity control mode");
        return ESP_ERR_INVALID_STATE;
    }

    if (odrive_ml_context->input_mode != INPUT_MODE_PASSTHROUGH || odrive_mr_context->input_mode != INPUT_MODE_PASSTHROUGH) {
        ESP_LOGE(ODOM_TASK_LOGGER_TAG, "ODrive is not in passthrough input mode");
        return ESP_ERR_INVALID_STATE;
    }
    ...
    if (servo_context == NULL) {
        ESP_LOGE(ODOM_TASK_LOGGER_TAG, "Invalid servo context");
        return ESP_ERR_INVALID_ARG;
    }
    s_servo_context = servo_context;

    s_odom_err_evt_group = error_handle;
    s_odom_err_bit = error_bit;
    xEventGroupClearBits(s_odom_err_evt_group, s_odom_err_bit);

    s_odom_evt_group = xEventGroupCreate();
    if (!s_odom_evt_group) {
        ESP_LOGE(ODOM_TASK_LOGGER_TAG, "Failed to create Odom event group");
        return ESP_FAIL;
    }

    xEventGroupSetBits(s_odom_evt_group, ODOM_TERM_BIT);
    xEventGroupClearBits(s_odom_evt_group, ODOM_RUN_BIT);
    ESP_LOGI(ODOM_TASK_LOGGER_TAG, "Event group created");

    s_odom_task_h = NULL;
    ESP_LOGI(ODOM_TASK_LOGGER_TAG, "Module initialized successfully");
    return ESP_OK;
}

esp_err_t odom_task_start(void) {
    if (!s_odom_task_h) {
        if (xTaskCreate(odom_task, ODOM_TASK_NAME, ODOM_TASK_STACK_SIZE, NULL, ODOM_TASK_PRIORITY, &s_odom_task_h) != pdPASS) {
            ESP_LOGE(ODOM_TASK_LOGGER_TAG, "Failed to create Odom task");
            s_odom_task_h = NULL;
            return ESP_FAIL;
        }
    }
    xEventGroupSetBits(s_odom_evt_group, ODOM_RUN_BIT);
    ESP_LOGI(ODOM_TASK_LOGGER_TAG, "Module started");
    return ESP_OK;
}

esp_err_t odom_task_stop(TickType_t wait_ticks) {
    if (!s_odom_task_h) {
        ESP_LOGI(ODOM_TASK_LOGGER_TAG, "Module not running");
        return ESP_OK; // Already stopped
    }

    ESP_LOGI(ODOM_TASK_LOGGER_TAG, "Stopping module...");
    xEventGroupClearBits(s_odom_evt_group, ODOM_RUN_BIT);

    EventBits_t bits = xEventGroupWaitBits(s_odom_evt_group, ODOM_TERM_BIT, pdFALSE, pdTRUE, wait_ticks);
    if (!(bits & ODOM_TERM_BIT)) {
        ESP_LOGE(ODOM_TASK_LOGGER_TAG, "Tasks did not stop in time");
        return ESP_FAIL;
    }
    ESP_LOGI(ODOM_TASK_LOGGER_TAG, "Module stopped");
    return ESP_OK;
}

esp_err_t odom_task_deinit(TickType_t wait_ticks) {
    if (odom_task_stop(wait_ticks) != ESP_OK) {
        return ESP_FAIL;
    }

    if (s_odom_evt_group) {
        vEventGroupDelete(s_odom_evt_group);
        s_odom_evt_group = NULL;
    }

    if (s_odom_task_h) {
        vTaskDelete(s_odom_task_h);
        s_odom_task_h = NULL;
    }
    ...
    s_odom_err_evt_group = NULL;

    ESP_LOGI(ODOM_TASK_LOGGER_TAG, "Module deinitialized");
    return ESP_OK;
}